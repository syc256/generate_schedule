...
# 配置日志记录器
logging.basicConfig(
    filename='error_log.txt',  # 指定日志文件名
    level=logging.ERROR,       # 设置日志级别为 ERROR（记录错误和更严重的消息）
...
for names in L:
    ...
    if len(matches) >= 3:
        ...
        logging.error(f"{names}表头格式错误，请检查xls文件是否从教务下载")

    # 检查打印时间
    date = table.row_values(1)[0][-10:]
    print(date[-10:])
    if date < cmpDate:
        logging.error(f"课表打印时间早于2023-09-01: {names},姓名:{name}")
